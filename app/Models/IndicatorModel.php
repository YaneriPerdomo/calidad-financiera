...
class indicatorModel extends Database
{
            ...
            foreach ($row_insome as $row) {
                ...
                if ($row['ingreso'] != 'Otros') {
                    $this->HTML_insome .= " 
                       ...
                       <button class='  button--delete' data-model='js'data-type='id_ingreso'  data-link='../../indicator/delete-insome'
                        data-id='".$row['id_ingreso']."' 
       >
        ...
        if ($total_records_i == 0) {
            ...
            if ($get_insome_stmt->rowCount() == 0) {
                ...
                for ($i = 1; $i <= $total_pages_i; $i++) {
                    $this->HTML_insome .= "<li class='page__item'><a href='./$i' class='page__link'>".($i == $current_page_i ? '<b  class="page__link--selected">'.$i.'</b>' : $i).'</a></li>  ';
                }
                if ($current_page_i < $total_pages_i) {
                    $this->HTML_insome .= "<li class='page__item'><a href='./".($current_page_i + 1)."'class='page__link'>Siguiente</a></li> ";
                } else {
            ...
            foreach ($row_graduation as $row) {
                ...
                if ($row['egreso'] != 'Otros gastos') {
                    ...
                    $this->HTML_graduantion .= "  
                       ...
                       <button class='  button--delete' data-model='js'data-type='id_egreso'  data-link='../../indicator/delete-graduation'
                        data-id='".$row['id_egreso']."' 
       >
                                <i class='bi bi-trash ' ></i> 
                              </button>
                         ";
    ...
    public function FindOneDate($table, $id, $id_field, $fields)
    {
    ...
    {
        $POST['type-indicator'] = $POST['type-indicator'] ?? 1;
        try {
    ...
    {
        $change_test_query = 'SELECT id_egreso FROM  transacciones WHERE id_egreso = :id_egreso ';
        $change_test_stmt = $this->pdo->prepare($change_test_query);
        $change_test_stmt->bindParam('id_egreso', $POST['id_egreso'], PDO::PARAM_INT);
        $change_test_stmt->execute();

        if ($change_test_stmt->rowCount() > 0) {
            $change_graduation_query = 'UPDATE transacciones SET id_egreso = 17 WHERE id_egreso = :id_egreso';
            $change_graduation_stmt = $this->pdo->prepare($change_graduation_query);
            $change_graduation_stmt->bindParam('id_ingreso', $POST['id_egreso'], PDO::PARAM_INT);
            $change_graduation_stmt->execute();
        }
    ...
    {
        $change_test_query = 'SELECT id_ingreso FROM  transacciones WHERE id_ingreso = :id_ingreso ';
        $change_test_stmt = $this->pdo->prepare($change_test_query);
        $change_test_stmt->bindParam('id_ingreso', $POST['id_ingreso'], PDO::PARAM_INT);
        $change_test_stmt->execute();
        if ($change_test_stmt->rowCount() > 0) {
            $change_insome_query = 'UPDATE transacciones SET id_ingreso = 7 WHERE id_ingreso = :id_ingreso';
            $change_insome_stmt = $this->pdo->prepare($change_insome_query);
            $change_insome_stmt->bindParam('id_ingreso', $POST['id_ingreso'], PDO::PARAM_INT);
            $change_insome_stmt->execute();
        }
